...
## Assumptions
-   **Event Time Input:** When creating an event, if `start_time` or `end_time` are provided as ISO 8601 strings without a timezone offset, they are assumed to be in the 'Asia/Kolkata' (IST) timezone. This is handled by the Pydantic schema validator.
-   **Storage Timezone:** All event `start_time` and `end_time` values are converted to and stored in UTC in the database.
...
-   **Upcoming Events:** An event is considered "upcoming" if its `end_time` (in UTC) is after the current UTC time.
...
-   **Unit Tests:** Comprehensive unit and integration tests are written using `pytest` and `HTTPX`, covering various scenarios including edge cases and timezone conversions. Located in the `tests/` directory.
